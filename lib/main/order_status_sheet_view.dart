...
class _ChatIconButtonState extends State<ChatIconButton> {
  Set<NewMessageReceived$Subscription$OrderMessage> newMessages = {};

  @override
  Widget build(BuildContext context) {
    return Mutation(
        options: MutationOptions(
            document: UPDATE_LAST_SEEN_MESSAGES_AT_MUTATION_DOCUMENT),
        builder: (RunMutation runMutation, QueryResult? result) {
          return Subscription<NewMessageReceived$Subscription>(
              options: SubscriptionOptions(
                  document: NEW_MESSAGE_RECEIVED_SUBSCRIPTION_DOCUMENT,
                  parserFn: NewMessageReceived$Subscription.fromJson),
              builder: (data) {
                if (data.parsedData != null &&
                    newMessages.firstWhereOrNull((p0) =>
                            p0.id == data.parsedData!.newMessageReceived.id) ==
                        null) {
                  newMessages.add(data.parsedData!.newMessageReceived);
                }
                return RoundedButton(
                    ...
                    onPressed: () async {
                      ...
                      await runMutation(UpdateLastSeenMessagesAtArguments(
