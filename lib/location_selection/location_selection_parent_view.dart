                          ...
                          onSubscriptionResult: (subscriptionResult, client) =>
                              {
                                if (subscriptionResult
                                        .parsedData?.orderUpdated !=
                                    null)
                                  {
                                    client?.cache.writeQuery(
                                        const Operation(
                                                document:
                                                    documentNodeSubscriptionUpdatedOrder)
                                            .asRequest(),
                                        data: {})
                                  }
                              },
                          builder: (result) {
                            // We used to pass this directly and the performance was great but some bug within either Apollo or GraphQL made us recall refetch every time. More specifically this bug results in all scalars (dates) to be null instead of parsing them internally.

                            if (result.hasException) {
                              print(result.exception.toString());
                            }
                            if (result.parsedData?.orderUpdated != null) {
                              final order =
                                  GetCurrentOrder$Query$Rider$Order.fromJson(
                                      result.parsedData!.orderUpdated.toJson());
